    ...
    assignGroup: async (playerId, group) => {
        ...
        if (group || group == null) {
            ...
            let jugadoresDelGrupo = await db.Jugador.findAll({where: {
                grupoId: group || 0 ,
                categoriaId: player.categoriaId
            }})
            if (jugadoresDelGrupo.length >= 4) { // Grupo lleno
                return 500;
            // } else if(model.findByGroup(group,player.categoriaId)) {

            } else {
                await db.Jugador.update({grupoId: group},{ where: {id: playerId}})
